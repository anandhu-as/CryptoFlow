  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 p-4">
      {coins.map((coin) => (
        ...
        >
          <img src={coin.image} alt={coin.name} className="w-12 h-12 object-contain drop-shadow-sm group-hover:scale-105 transition-transform" />

          <div className="text-center">
            <h2 className="text-lg font-bold text-zinc-900 dark:text-white leading-tight">{coin.name}</h2>
            <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider mt-0.5">{coin.symbol}</p>
          </div>
